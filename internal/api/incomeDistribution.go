...
type IncomeReportResponse struct {
	...
	IncomeData []IncomeReportItem `json:"incomeData"`
}

func incomeDistribution(w http.ResponseWriter, r *http.Request) {
	enableCORS(w, r)
	...
	var incomeItems []IncomeReportItem
	totalAmount := 0.0
	totalCurrency := "USD"

	// Iterate over subreports to find Revenues / Income
	for _, sub := range cbrSubreports {
		...
		// Accept "Revenues" or anything starting with "income"
		if !strings.EqualFold(subName, "Revenues") && !strings.HasPrefix(strings.ToLower(subName), "income") {
			continue
		}
		...
		for _, row := range prRows {
			...
			totalAmount += amount
			incomeItems = append(incomeItems, IncomeReportItem{
				Account:  accountName,
				Amount:   amount,
				Currency: currency,
			})
